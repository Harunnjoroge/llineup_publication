...
#working directory
wd <- "~/lstm_scratch/network_scratch/llineup/llineup-genomics"
setwd(wd)
...
#gwas round 1 vs 5
dir.glm.result <- './glm/allnets_results'  #set the rounds you want 
...
for (c in genotype.files){
  ...
  gt  <- readRDS(c)
  setwd(wd)
  metadata <- fread('./glm/metadata/merged_llineup_metadata.csv',na.strings = '') # load your metadata
  ...
  #Prep data-Select pbo or npbo samples in round 1 and 5 or any net and rounds you want
 #metadata_pbo<- metadata[sample_id %in% sample.names & sex_call=='F' & RND %in% c(1,5)&LLIN.actual=='PBO LLIN']

  #metadata_npbo<- metadata[sample_id %in% sample.names & sex_call=='F' & RND %in% c(1,5)&LLIN.actual=='Non-PBO LLIN']
  ...
  model.meta<- metadata[,.(sample_id= rep(sample_id, each =2),
                           ...
                           LLIN.actual = rep(llin_actual, each = 2),
