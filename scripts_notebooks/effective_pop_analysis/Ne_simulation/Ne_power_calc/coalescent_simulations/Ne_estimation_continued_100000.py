...
vcf_files = list(filter(lambda x: search('\\d\\.vcf', x) , allfiles))
...
for popsize in popsizes:
	...
	# Subset the vcf to keep 10% of SNPs, for faster computation
	print('\tSubsetting vcfs to keep only 10% of variants')
	sys.stdout.flush()
	crash_files_subset = [sub('\\.vcf$', '_subset.vcf', x) for x in crash_files]
	nocrash_files_subset = [sub('\\.vcf$', '_subset.vcf', x) for x in nocrash_files]
	for i in range(len(crash_files)):
		os.system("awk '/^#/ { print; next } ++n % 10 == 0' " + crash_files[i] + " > " + crash_files_subset[i])
		os.system("awk '/^#/ { print; next } ++n % 10 == 0' " + nocrash_files[i] + " > " + nocrash_files_subset[i])

	print('\tLoading vcf')
	sys.stdout.flush()
	callsets_crash = [allel.read_vcf(f) for f in crash_files_subset]
	callsets_nocrash = [allel.read_vcf(f) for f in nocrash_files_subset]
