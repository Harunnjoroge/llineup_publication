...
def simulate_and_save(currentN, recomb, demog, n_rep, note = '', should_save = True):
	if demog is None:
		...
		tree_sequences = msprime.simulate(Nh, Ne = currentN, recombination_map = recomb, num_replicates = n_rep, model = 'dtwf')
		...
		tree_sequences = msprime.simulate(Nh, Ne = currentN, recombination_map = recomb, demographic_events = demog, num_replicates = n_rep, model = 'dtwf')
...
startN_small = 100
crashN_small = int(round(startN_small/10, 3))
startN_large = 1000
crashN_large = int(round(startN_large/10, 3))

# First the small population
demog_small = [msprime.PopulationParametersChange(time = crash_t, initial_size = startN_small, growth_rate = 0)]
dd_small = msprime.DemographyDebugger(Ne = crashN_small, demographic_events = demog_small)
print('\nDemographic history for crash in smaller population (' + str(startN_small) + ' to ' + str(crashN_small) + ').')
dd_small.print_history()

# Simulate the populations before the crash
tree_sequences_small_nocrash = simulate_and_save(startN_small, recomb, None, num_rep, '_beforecrash')
# And after the crash
tree_sequences_small_crash = simulate_and_save(crashN_small, recomb, demog_small, num_rep, '_crash')

# Now the large population
demog_large = [msprime.PopulationParametersChange(time = crash_t, initial_size = startN_large, growth_rate = 0)]
dd_large = msprime.DemographyDebugger(Ne = crashN_large, demographic_events = demog_large)
print('\nDemographic history for crash in larger population (' + str(startN_large) + ' to ' + str(crashN_large) + ').')
dd_large.print_history()

# Simulate the populations before the crash
tree_sequences_large_nocrash = simulate_and_save(startN_large, recomb, None, num_rep, '_beforecrash')
# And after the crash
tree_sequences_large_crash = simulate_and_save(crashN_large, recomb, demog_large, num_rep, '_crash')
